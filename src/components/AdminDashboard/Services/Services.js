...
function Services() {
  ...
  useEffect(() => {
    const getAllOrders = async () => {
      const fetchOrders = await axios.get(`https://techsoldiers.herokuapp.com/services`);
  ...
  const deleteService = async (id) => {
    try {
      const deletedService = await axios.get(`https://techsoldiers.herokuapp.com/deleteService/${id}`);
